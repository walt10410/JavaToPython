...
print("Hello World!") # Remove after, let me know running

#************CONSTENTS & INITIAL VALUES
microcontroller.enable_interrupts
DEBUG = True
serial = usb_cdc.data
serial.timeout : 0.1 #seconds
usb_cdc.console.timeout : 0.1 #seconds

in_data = bytearray()
out_data = b'waiting****'

USB_Index = -1 # DO NOT CHANGE
Ch_Select = array.array('I', [188,0,0,0,0,0,0,0,0,2]) # unsigned interger array each item is 2 bytes
bUSB_read= True
...
#Ch[5]	Relay 5	Stop Tram
#Ch[6]	Relay 6	Lights
#Ch[7]	Relay 7	OPEN
#Ch[8]	Relay 8	OPEN

# Ch[9]	open	must be integer 1 through 9
#%%%%%%%%%%%%%%%
...
#%%%%START LOOP%%%%
while True:
    while serial.in_waiting > 0: # data waiting in buffer
        USB_Index += 1      #starts at 0, initial value is -1
        bUSB_read = True	# used to id change for new input for writing to relays
        ...
        #start of if
        if serial_bit == b'\n':		# Found end of line
            serial.write(in_data)	# write data back
            time.monotonic(25)	# Allow system to write the code for 25 ms?
            
            log(in_data.decode("utf-8")) # check for errors
            out_data = in_data
            print(in_data)
...
#^^ if read data then write outputs^^
    ...
    if bUSB_read:	# input and write output here
            ...
            #**** Relays set, now wait and turn off Train Switch Solenoids
            if Ch_Select[9] < 1: Ch_Select[9] = 1 # this is multiplier, must be between 1 and 9
            time.monotonic(Ch_Select[0]*Ch_Select[9]) # delay ms timer so switch will move and stabalize, then turn off all relays
            ch1.value = False
            ch2.value = False
            ch3.value = False
            ch4.value = False
            ch5.value = False	# Stop Tram at station
            ch6.value = False	# board lights
            ch7.value = False	# on Relay 9 through 16 this is open
            ch8.value = False	# on Relay 9 through 16 this is open
            bUSB_read = False	# trigger to output write
    ...
    # time delays
    time.monotonic(10)   # ms timer to allow pico to do other things ??Maybe can delete??
